-- ═══ Fix: el saldo a favor (crédito) debe aplicarse en la boleta nueva ═════
-- La migración 20260728_04 había quitado el clamp `if v_saldo_ant < 0 then 0`,
-- pero los rewrites posteriores de confirmar_pedido (20260728_07 fecha_reparto
-- y 20260729_02 costo_snapshot) lo volvieron a meter sin querer. Resultado:
-- un cliente con saldo a favor (negativo) veía 0 de saldo anterior en la boleta
-- nueva, mientras que el saldo en contra sí se reflejaba.
--
-- Este rewrite = base 20260729_02 (con snapshot + fecha_reparto por zona) pero:
--   - sin el clamp: v_saldo_ant conserva el negativo (crédito).
--   - total_a_cobrar = greatest(v_total + v_saldo_ant, 0) para no quedar bajo 0.
...
begin
  ...
  -- saldo previo: SIN clamp — un saldo a favor (negativo) se conserva y
  -- descuenta del total a cobrar de esta boleta.
  v_saldo_ant := coalesce(
    (select saldo from saldo_actual where cliente_id = v_cliente_id),
    0
  );

  v_total_cobrar := greatest(v_total + v_saldo_ant, 0);
  ...
  for v_item in select * from jsonb_array_elements(v_items) loop
    ...
    insert into pedido_items
      (empresa_id, pedido_id, producto_id, cantidad, precio_unit, descuento,
       es_bulto, u_por_bulto, oferta_id, costo_unit_snapshot)
    values
