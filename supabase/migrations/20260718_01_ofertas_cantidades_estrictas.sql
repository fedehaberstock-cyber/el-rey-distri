-- ═══ Ofertas: cantidades estrictas (obligatorias) vs máximo ══════════════
-- Antes cantidad_min era el mínimo del principal y regalo_cantidad era la
-- cantidad fija a agregar. Ahora agregamos un flag: si estrictas=true, las
-- cantidades cargadas son obligatorias (se agregan exactas al aplicar). Si
-- estrictas=false (default), son el mínimo/máximo y al aplicar se pregunta.

alter table public.ofertas
  add column if not exists cantidades_estrictas boolean not null default false;

comment on column public.ofertas.cantidades_estrictas is
  'Si true, al aplicar la oferta se agregan exactamente cantidad_min + regalo_cantidad sin pedir input. Si false, se pregunta al preventista.';

-- Actualizar RPC de sugerencias para devolver el campo
