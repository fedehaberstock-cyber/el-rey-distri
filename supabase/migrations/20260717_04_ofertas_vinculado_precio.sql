-- ═══ Ofertas: producto vinculado con precio promocional opcional ══════════
-- Antes el "regalo" era siempre 100% off. Ahora podés poner un precio
-- promocional (ej: llevá 1 Kolinos y podés llevar hasta 6 Colgate a $X).
-- Si regalo_precio es null → sigue siendo gratis (comportamiento anterior).

alter table public.ofertas
  add column if not exists regalo_precio numeric;

comment on column public.ofertas.regalo_precio is
  'Precio unitario del producto vinculado. NULL = gratis (100% off).';

-- Actualizar RPC de sugerencias para incluir regalo_precio en el payload
