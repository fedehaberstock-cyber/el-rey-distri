-- ═══ Fix: restaurar costo_unit_snapshot en confirmar_pedido ══════════════
-- Desde la migración de ofertas (20260716_04) los rewrites sucesivos de
-- confirmar_pedido perdieron el `costo_unit_snapshot` al insertar en
-- pedido_items. Todos los pedidos creados desde entonces quedaron con
-- snapshot NULL → reporte_ventas cae al costo actual del producto (impreciso).
--
-- 1) Backfill de los items sin snapshot (usan costo actual — mejor que nada).
-- 2) Rewrite de confirmar_pedido incluyendo el snapshot en el insert.
...
-- 2) RPC con snapshot restaurado (base = 20260728_07 + snapshot)
