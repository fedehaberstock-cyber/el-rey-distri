...
-- La migración 20260716_04 (confirmar_pedido con soporte de ofertas) volvió a
-- declarar v_saldo_ant sin inicializar. Cuando el cliente no tiene filas en
-- saldo_actual (cliente nuevo), SELECT INTO no asigna nada y v_saldo_ant queda
-- NULL → rompe el NOT NULL de boletas.saldo_anterior.
-- Repone el patrón: v_saldo_ant := 0 en la declaración + check adicional.
...
begin
  ...
  -- saldo previo (subquery escalar con coalesce cubre el caso "sin filas")
