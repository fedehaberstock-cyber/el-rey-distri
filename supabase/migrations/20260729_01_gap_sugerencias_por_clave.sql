-- ═══ Fix gap_sugeridos: excluir por CLAVE (subcategoría/nombre), no por producto_id ══
-- Bug en 20260718_01: el `not exists` comparaba `pi2.producto_id = pr.id`,
-- entonces si el cliente compró UN producto de una subcategoría, los otros
-- productos de la misma subcategoría igual aparecían como gap.
-- Fix: excluir la CLAVE completa cuando el cliente compró cualquier producto
-- que pertenece a esa clave en la ventana de 90 días.
...
begin
  ...
  prods as (
    select pi.producto_id, count(distinct u.id) as apariciones
  ...
  -- Gap por CLAVE (subcategoría o nombre del producto).
  -- Se excluyen las claves que el cliente ya compró en los últimos 90 días
  -- (mirando la subcategoría/nombre, no el producto puntual).
  with claves_cliente as (
    select distinct coalesce(nullif(btrim(pr2.subcategoria),''), pr2.nombre) as clave
     ...
     where p2.cliente_id = p_cliente_id
       and p2.empresa_id = v_emp
       and p2.estado <> 'anulado'
       and p2.fecha > current_date - 90
  )
    ...
    from (
      select coalesce(nullif(btrim(pr.subcategoria),''), pr.nombre) as clave,
       ...
       where p.fecha > current_date - 90
         ...
         and coalesce(nullif(btrim(pr.subcategoria),''), pr.nombre)
             not in (select clave from claves_cliente)
       group by clave, tipo
  ...
  with recent_ingresos as (
    select producto_id, max(fecha) as fecha_ingreso
      from mov_stock
     where tipo = 'ingreso' and fecha >= (now() - interval '7 days')
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'cantidades_estrictas', t.cantidades_estrictas,
    ...
    'regalo_precio',      t.regalo_precio,
    'regalo_precio_normal', t.regalo_precio_normal,
    ...
    from (
      select o.id, o.producto_id, pr.nombre, pr.categoria, pr.subcategoria,
             ...
             o.cantidad_min, o.cantidades_estrictas, o.aplica_desc_bulto,
             pr.u_bulto, pr.desc_bulto,
             o.regalo_producto_id, prr.nombre as regalo_nombre, o.regalo_cantidad,
             o.regalo_precio, prr.precio as regalo_precio_normal,
